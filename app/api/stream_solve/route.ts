...
export async function POST(req: Request) {
  const { links, taskId } = await req.json();
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // Accumulate final results per link for Firebase persistence
      ...
      // ===== PERSIST TO FIREBASE =====
      // Save all accumulated results with logs to Firestore
      if (taskId) {
        try {
          ...
          if (taskDoc.exists) {
            ...
            // Update each link with its final result + logs
