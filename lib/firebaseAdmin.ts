...
// 1. Apni details direct yahan daal dete hain taaki Vercel ko mil jaye
const projectId = "bhaag-df531";
const clientEmail = "firebase-adminsdk-5pplx@bhaag-df531.iam.gserviceaccount.com";

// Private key ko bilkul safe format mein banaya hai
...
// 3. SAFE DB EXPORT: Safety Guard lagaya hai taaki crash na ho
let db: any = null;
try {
  db = admin.firestore();
} catch (error) {
  console.error("❌ Firestore start nahi ho paya:", error);
}
